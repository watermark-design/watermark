...
<script lang="ts">
  import { defineComponent, isVue2, CSSProperties, PropType } from 'vue-demi';
  ...
  export default defineComponent({
    ...
    computed: {
      options(): Partial<WatermarkOptions> {
        return {
          ...initialOptions,
          ...(this.$props as Partial<WatermarkOptions>),
        };
      },
    ...
    methods: {
      ...
      async draw() {
        this.mutationObserve &&
          ((this.$refs.watermarkWrapperRef as HTMLElement).id = `watermark-${this.key}`);
        await this.watermarkCanvas?.draw();
        const layoutCanvas = renderLayout(
          this.options as WatermarkOptions,
          this.watermarkCanvas?.getCanvas() as HTMLCanvasElement
        );
      ...
      addObserver() {
        const watermarkWrapperDom = this.$refs.watermarkWrapperRef as HTMLElement;
        const parentElement = watermarkWrapperDom.parentElement as HTMLElement;
